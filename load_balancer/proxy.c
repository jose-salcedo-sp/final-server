...
void handle_client_connection(int client_socket_fd, char *backend_host,
  ...
  log_info("Resolved backend %s:%s", backend_host, backend_port_str); // ✅ LOG
  ...
  for (addrs_iter = addrs; addrs_iter != NULL;
    ...
    if (connect(backend_socket_fd, addrs_iter->ai_addr,
      ...
      log_success("Connected to backend %s:%s", backend_host,
                  backend_port_str); // ✅ LOG
      break;
    }
  ...
  log_info("Read %d bytes from client", bytes_read); // ✅ LOG
  write(backend_socket_fd, buffer, bytes_read);
  log_info("Forwarded %d bytes to backend", bytes_read); // ✅ LOG

  while ((bytes_read = read(backend_socket_fd, buffer, BUFFER_SIZE)) > 0) {
    log_info("Read %d bytes from backend", bytes_read); // ✅ LOG
    write(client_socket_fd, buffer, bytes_read);
    log_info("Sent %d bytes to client", bytes_read); // ✅ LOG
  }

  log_success("Connection closed"); // ✅ LOG
...
int main(int argc, char *argv[]) {
  ...
  log_info("Starting proxy on port %s", server_port_str); // ✅ LOG
  memset(&hints, 0, sizeof(struct addrinfo));
  hints.ai_family = AF_UNSPEC;
  hints.ai_socktype = SOCK_STREAM;
  ...
  for (addr_iter = addrs; addr_iter != NULL; addr_iter = addr_iter->ai_next) {
    ...
    if (bind(server_socket_fd, addr_iter->ai_addr, addr_iter->ai_addrlen) ==
        0) {
      log_success("Bound to port %s", server_port_str); // ✅ LOG
      break;
    }
  ...
  log_info("Listening on port %s", server_port_str); // ✅ LOG
  ...
  while (1) {
    ...
    log_info("Accepted new client connection"); // ✅ LOG
